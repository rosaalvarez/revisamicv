...
test('buildRevisionSystemPrompt permits user corrections but blocks invented experience', async () => {
  ...
  assert.match(prompt, /para cuidar tu credibilidad/i)
  assert.match(prompt, /supportive coaching/i)
  assert.match(prompt, /Never use moralizing or judgmental language/i)
  assert.match(prompt, /safe alternative/i)
